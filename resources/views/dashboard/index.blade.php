...
@section('content')
    ...
    <div class="p-4 sm:ml-64">
        <p class="text-2xl mb-4 font-bold ">Dashboard</p>
    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 border-2 flex flex-col border-gray-200 border-dashed rounded-lg dark:border-gray-700">
            <p class="text-lg mb-4 font-semibold">Gambar Depan</p>

            {{-- File Upload Area --}}
            <form method="POST" action="{{ route('upload') }}" enctype="multipart/form-data" class="flex flex-row gap-4">
                @csrf
                <input type="hidden" name="type" value="dashboard">
                <input name="file"
                    class="block max-w-sm  w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
            ...
            <div class="mt-4 overflow-x-auto">
                <table class="w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-700 dark:divide-gray-600">
                        @forelse ($dashboardImages as $img)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900 dark:text-white">{{ $img->original_name ?? 'N/A' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900 dark:text-white">
                                        {{ number_format($img->size / 1024, 2) }} KB</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                    {{ $img->created_at->format('M d, Y') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <form action="{{ route('upload.updateOrder', $img->id) }}" method="POST"
                                        class="flex items-center">
                                        @csrf
                                        @method('PATCH')
                                        <input type="number" name="order" value="{{ $img->order }}"
                                            class="w-16 text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                            min="0">
                                        <button type="submit"
                                            class="ml-2 text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        ...
                                        <form action="{{ route('upload.destroy', $img->id) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 border-2 flex flex-col border-gray-200 border-dashed rounded-lg dark:border-gray-700">
            <p class="text-lg mb-4 font-semibold">Gambar Layanan Mudik</p>

            {{-- File Upload Area --}}
            <form method="POST" action="{{ route('upload') }}" enctype="multipart/form-data"
                class="flex flex-row gap-4">
                @csrf
                <input type="hidden" name="type" value="layanan-mudik">
                <input name="file"
                    class="block max-w-sm  w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
                    id="file_input" type="file">
            ...
            <div class="mt-4 overflow-x-auto">
                <table class="w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-700 dark:divide-gray-600">
                        @forelse ($layananMudikImages as $img)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-20 w-20">
